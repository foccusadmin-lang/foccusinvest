...
export function BonusHistorico({ itens }: { itens: Credito[] }) {
  const { grupos, expandidos, toggle } = useGrupoPorData(itens);

  if (grupos.length === 0) {
    return (
      <p className="rounded-2xl border border-border bg-surface p-6 text-center text-sm text-muted">
        Você ainda não recebeu bônus de indicação.
      </p>
    );
  }
              ...
              <div className="divide-y divide-border">
                {grupoItens.map((c) => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        <p className="font-semibold text-fuchsia-300">{formatMoeda(c.valor)}</p>
